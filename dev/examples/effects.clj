(ns examples.effects
  ...
  (:import [io.github.humbleui.skija BlendMode Canvas Paint Shader]))

@ui/button ;; FIXME

(util/deftype+
  Ripple
  [^:mut center ^:mut progress-start ^:mut hovered?]
  :extends
  ui/AWrapperNode
  protocols/IComponent
  (-draw-impl
    [_ ctx bounds viewport ^Canvas canvas]
    (let [{:keys [scale window]} ctx
          [_ opts _] (ui/parse-element element)
          {:keys [duration radius state], :or {duration 128, radius 1.5}} opts
          pressed? (:pressed state)
          border-radius 10
          rrect (util/rrect-xywh (:x bounds)
                                 (:y bounds)
                                 (:width bounds)
                                 (:height bounds)
                                 (* scale border-radius))
      ...
      (canvas/with-canvas
        ...
        (when (< progress 1) (.drawPaint canvas ui/button-bg))
        (when (> progress 0)
          (canvas/draw-circle
            canvas
            (:x center)
            (:y center)
            (* max-r progress)
            (if pressed? ui/button-bg-pressed ui/button-bg-hovered)))
        (ui/draw child ctx bounds viewport canvas)
        (when (< 0 progress 1) (window/request-frame window)))))
...
(defn ripple [opts child] (map->Ripple {}))

(defn button
  [opts child]
  [ui/clickable {}
   (fn [state] [ripple (assoc opts :state state)
                [ui/padding {:horizontal 40, :vertical 20}
                 [ui/center child]]])])

;; card

(def default-gradient
  (Shader/makeLinearGradient (float 0)
                             (float 0)
                             (float 1)
                             (float 1)
...
(util/deftype+
  ...
  (-draw-impl
    ...
    (let [{:keys [mouse-pos scale]} ctx
          [_ opts _] (ui/parse-element element)
          {:keys [bg gradient],
           :or {bg (paint/fill 0xFF202020), gradient default-gradient}}
            opts]
      ...
      (util/when-some+
        ...
        (when (= :mouse-move (:event event))
          (let [hovered?' (util/rect-contains? bounds (util/ipoint x y))]
            (util/eager-or
              (when (not= hovered? hovered?') (set! hovered? hovered?') true)
              (when hovered?' true)))))
      (ui/event child ctx event))))

(defn card [opts child] (map->Card {}))
   ...
   [ui/vscroll
    [ui/align {:x :center}
     [ui/padding {:padding 20}
      [ui/column {:gap 10} [button {} [ui/label "Hover ripple"]]
       [ui/clip {:radius 10}
        [card {}
         [ui/with-context {:fill-text (paint/fill 0xFFFFFFFF)}
          [ui/padding {:horizontal 30, :vertical 20}
           [ui/size {:width 170}
            [ui/column [ui/label {:font-weight :bold} "Card title"]
             [ui/gap {:height 20}]
             [ui/paragraph {:font-cap-height 10, :line-height 20}
              "Just some text here to let you see and appreciate how highlight gradient works on this card"]]]]]]]]]]]])
